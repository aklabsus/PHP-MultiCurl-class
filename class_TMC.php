...
class TMC{
	...
	static $maxConnections=5;	// одновременно выполняющиеся запросы
	static $OPT_TIMEOUT=60;
	...
	static function Process(){
		...
		while(false!==($x=curl_multi_info_read(self::$mch))){	// читаем сообщения
			...
			if(isset($APPLICATION)) $APPLICATION->processMessages();
		}
		// self::CH_finished
		return true;
	}
	...
	static function NewThread($ARRAY, &$EndFunc=null, $Pars=array()){
		$ch1=curl_init();
		...
		curl_setopt($ch1, CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows NT 6.1; rv:13.0) Gecko/20100101 Firefox/13.0.1'); 
